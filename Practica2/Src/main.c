...
static const tick_t duration_led2 = 500;
static const tick_t duration_led3 = 1000;

static case_led_t case_led = LED_CASE1;
...
int main(void) {
	...
	while (1) {
		/*
		 The three LEDs are made to blink periodically and independently.
		...
		//Add to test delayWrite

		if (BSP_PB_GetState(BUTTON_USER)) {
			switch (case_led) {
			case LED_CASE1: {
				case_led = LED_CASE2;
				delayWrite(&delay_LED1, duration_led1);
				delayWrite(&delay_LED2, duration_led2);
				delayWrite(&delay_LED3, duration_led3);
				break;
			}
			case LED_CASE2: {
				case_led = LED_CASE3;
				delayWrite(&delay_LED1, duration_led2);
				delayWrite(&delay_LED2, duration_led3);
				delayWrite(&delay_LED3, duration_led1);
				break;
			}
			case LED_CASE3: {
				case_led = LED_CASE1;
				delayWrite(&delay_LED1, duration_led3);
				delayWrite(&delay_LED2, duration_led1);
				delayWrite(&delay_LED3, duration_led2);
				break;
			}
			}
		}
