...
static const uint8_t SECUENCE_MAX = sizeof(secuence) / sizeof(Led_TypeDef);

static uint8_t index = 0;
static const uint32_t delay = 200; // 50 200 1000
...
int main(void) {
	...
	delayInit(&delay_LED1, delay);
	delayInit(&delay_LED2, delay);
	delayInit(&delay_LED3, delay);
...
void index_change(void) {
	if (direction == CLOCKWISE) {
		index++;
		if (index == SECUENCE_MAX)
			index = 0;
	} else {
		if (index == 0)
			index = SECUENCE_MAX;
